...
main(int argc, char* argv[]) {
  ...
  string shell_prefix1 = "mysh (";
  string shell_prefix2 = ")> ";
  ...
  for (i = 0; i < recordsLeft; i++) {
    ...
    output << shell_prefix1 + to_string(i+1) + shell_prefix2 + word << endl;
    dup.insert(word);
  }

  input << "exit" << endl;
  output << shell_prefix1 + to_string(i+1) + shell_prefix2;
  input.close();
  output.close();

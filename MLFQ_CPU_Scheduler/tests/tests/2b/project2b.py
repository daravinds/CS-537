...
class Test5(Xv6Test):
   name = "stress_test_io"
   description = """Workload: Run multiple concurrent i/o requests
  Expected: No regression in i/o and scheduler does not fail"""
   tester = name + ".c"
   timeout = 30
   point_value = 10
   make_qemu_args = "CPUS=1"
...
class Test8(Xv6Test):
   name = "multiple_jobs_io"
   description = """Workload: Just checks i/o functionality still works
  Expected: No regression in i/o and scheduler does not fail"""
   tester = name + ".c"
   timeout = 30
   point_value = 10
...
'''class TestHidden2(Xv6Test):
   name = "relinquish"
   description = """Workload: 4 processes running long running workloads, but one of them yields CPU for a while and regains CPU
  Expected: The process that yields CPU should stop its timer ticks until it regains CPU"""
   tester = name + ".c"
   timeout = 120
   point_value = 15
   make_qemu_args = "CPUS=1"'''

released_tests = [ Test0, Test1, Test2, Test3, Test4, Test5, Test6, Test7, Test8, Test9, Test10 ]
